import { AjaxManager, requests } from './AjaxManager'
import NodeManager from './NodeManager'
import Answer from './Answer'
import plupload from '../../../../../../PlUploadBundle/Resources/public/plupload.full.min'
import $ from 'jquery'

const LocalSidePanel = function () {
  const _self = this
  ...
  this.$conditionAdd.on("click", function (e) {
    e.preventDefault()
    const Node = NodeManager.get(_self.selectedNodeID)

    requests.addCondition.ops.successFn = function (response) {
      Node.addCondition(response)
    }
    requests.addCondition.submit({
...
LocalSidePanel.prototype.show = function (nodeID) {
  ...
  this.disableInputs()
  this.load()
  requests.getPage.ops.successFn = function (nodeData) {
    Node.nodeData = nodeData
    _self.load()
    _self.enableInputs()
  }
  requests.getPage.submit({}, requests.getPage.url + this.selectedNodeID)
}
...
LocalSidePanel.prototype.load = function () {
  ...
  if (Node.nodeData.questions.length > 0) {
    $.each(Node.nodeData.questions[ 0 ].answerOptions, function () {
      // question here will be just an ID, need to re-populate it, data would have infinite loop if it had the question in it again. When adding a new one, the question is in the data
      const aOps = Object.assign({}, this, { question: Node.nodeData.questions[ 0 ] })
      _self.addAnswer(aOps)
    })
  }
}
...
export default new LocalSidePanel()
